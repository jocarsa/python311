...
def editaTabla(tabla):
    ...
    marco.pack()
def insertar():
    for widget in marcoinsertar.winfo_children():
        widget.destroy()
    print("vamos a insertar un nuevo registro en la tabla "+tablaactual)
    marco.pack_forget()
    marcoinsertar.pack()
    cursor = conexion.cursor()
    cursor.execute('''
    PRAGMA table_info("'''+tablaactual+'''");
    ''')
    datos = cursor.fetchall()
    listacolumnas = []
    for registro in datos:
    ...
    for columna in listacolumnas:
        tk.Label(marcoinsertar,text=columna).pack()
        tk.Entry(marcoinsertar).pack()
    tk.Button(marcoinsertar,text="Envía").pack()
#Ventana TKinter
raiz = tk.Tk()
...
botoninsertar = tk.Button(text="+",command=insertar)
botoninsertar.pack()
...
arbol = ttk.Treeview(marco, columns=columnas, show='headings')
arbol.pack()
...
marcoinsertar = tk.Frame()
textoinsertar = tk.Label(marcoinsertar,text="Vamos a insertar un registro")
textoinsertar.pack()
